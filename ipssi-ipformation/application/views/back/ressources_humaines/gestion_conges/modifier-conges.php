<div class="container">
	<div class="row">
		<div class="col-xs-12">
			<h1 class="titrePage center">Modifier une demande de congés</h1>
		</div>
	</div>
	...
	<?php echo form_open('/ipssi/ressources-humaines/liste-conges/modifier/'.$conge->id_conges); ?>
		...
		<div class="form-group">
			...
			<select id="type_conges" name="type_conges" class="form-control" onchange="affichageFormulaire(this.value);">
				...
				<?php foreach($type_conges as $t) : ?>
					<?php if(set_value('type_conges', $conge->id_type_conges) == $t->id_type_conges) : ?>
						<option value="<?php echo $t->id_type_conges ?>" selected><?php echo $t->libelle ?></option>
				<?php else :?>
		...
		<?php echo form_error('etat', '<div class="alert alert-danger">', '</div>'); ?>
		<div class="form-group">
		...
		<div class="form-group">
			...
			<input type="text" class="form-control" id="date_debut" placeholder="Date de début de la demande : dd-mm-yyyy" name="date_debut" value="<?php echo set_value('date_debut', $conge->date_debut); ?>">
		</div>

		<?php echo form_error('date_fin', '<div class="alert alert-danger">', '</div>'); ?>
		<div class="form-group">
			<label for="date_fin">Date de fin de la demande</label>
			<input type="text" class="form-control" id="date_fin" placeholder="Date de fin de la demande : dd-mm-yyyy" name="date_fin" value="<?php echo set_value('date_fin', $conge->date_fin); ?>">
		</div>

		<?php echo form_error('nb_jour', '<div class="alert alert-danger">', '</div>'); ?>
		<div class="form-group">
			<label for="nb_jour">Nombre de jours</label>
			<input type="text" class="form-control" id="nb_jour" placeholder="Nombre de jours" name="nb_jour" value="<?php echo set_value('nb_jour', $conge->nb_jour); ?>">
		</div>
