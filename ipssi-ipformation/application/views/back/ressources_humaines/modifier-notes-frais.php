<div class="container">
	<div class="row">
		<div class="col-xs-12">
			<h1 class="titrePage center">Modifier une note de Frais</h1>
		</div>
	</div>
	...
	<?php echo form_open('/ipssi/ressources-humaines/note-frais/modifier/'.$note_frais->id_note_frais); ?>
		...
		<div class="form-group">
			...
			<select id="type_note_frais" name="type_note_frais" class="form-control" onchange="affichageFormulaire(this.value);">
				...
				<?php foreach($type_note_frais as $t) : ?>
					<?php if(set_value('type_note_frais', $note_frais->id_type_note_frais) == $t->id_type_note_frais) : ?>
						<option value="<?php echo $t->id_type_note_frais ?>" selected><?php echo $t->libelle_type_note_frais ?></option>
				<?php else :?>
		...
		<div class="form-group">
			...
			<textarea class="form-control" id="description" name="description" rows="5"><?php echo set_value('description', $note_frais->description_note_frais); ?></textarea>
		</div>
		<?php echo form_error('date_note', '<div class="alert alert-danger">', '</div>'); ?>
		<div class="form-group">
			<label for="date_note">Date de la note de frais</label>
			<input type="text" class="form-control" id="date_note" placeholder="Date de la note de frais : dd-mm-yyyy" name="date_note" value="<?php echo set_value('date_note', $note_frais->date_note_frais); ?>">
		</div>
		<?php echo form_error('montant', '<div class="alert alert-danger">', '</div>'); ?>
		<div class="form-group">
			<label for="montant">Montant</label>
			<input type="text" class="form-control" id="montant" placeholder="Montant (en €)" name="montant" value="<?php echo set_value('montant', $note_frais->montant_note_frais); ?>">
		</div>
		...
		<div id="AAfficher">
			...
			<div class="form-group">
				...
				<input type="text" class="form-control" id="trajet" placeholder="Trajet" name="trajet" value="<?php echo set_value('trajet', $note_frais->trajet_note_frais); ?>">
			</div>
			<?php echo form_error('km_parcouru', '<div class="alert alert-danger">', '</div>'); ?>
			<div class="form-group">
				<label for="km_parcouru">Nombre de km parcourus</label>
				<input type="text" class="form-control" id="km_parcouru" placeholder="km Parcourus" name="km_parcouru" value="<?php echo set_value('km_parcouru', $note_frais->km_parcouru_note_frais); ?>">
			</div>
			<?php echo form_error('prix_km', '<div class="alert alert-danger">', '</div>'); ?>
			<div class="form-group">
				<label for="prix_km">Prix par km</label>
				<input type="text" class="form-control" id="prix_km" placeholder="Prix / km" name="prix_km" value="<?php echo set_value('prix_km', $note_frais->montant_km_note_frais); ?>">
			</div>
		</div>
		<div class="form-group">
